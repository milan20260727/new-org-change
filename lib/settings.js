...
async function getLastChangeLogExportAt() {
  const items = await fetchSourceRecords('settings');
  const row = items[0];
  const raw = row ? extractText(row.fields['Last Change Log Export At']) : null;
  return raw ? Number(raw) : 0;
}

// epochMs is the high-water mark to save — everything at or before it is considered archived.
async function setLastChangeLogExportAt(epochMs) {
  const items = await fetchSourceRecords('settings');
  const row = items[0];
  const fields = { 'Last Change Log Export At': String(epochMs) };
  if (row) {
    await updateSourceRecord('settings', row.record_id, fields);
  } else {
    await createSourceRecord('settings', fields);
...
module.exports = { getEditWindow, setEditWindow, getLastChangeLogExportAt, setLastChangeLogExportAt };
