...
const { fetchSourceRecords, createSourceRecord, deleteSourceRecords } = require('./fetchLark');
...
// Admin-only, irreversible: wipes the shared feed for everyone. Doesn't touch anyone's local
// `log` (their own not-yet-exported draft) — only the shared mirror.
async function clearChangeLog() {
  const items = await fetchSourceRecords('changelog');
  const ids = items.map((item) => item.record_id);
  await deleteSourceRecords('changelog', ids);
  return ids.length;
}

module.exports = { listChangeLog, appendChangeLogEntry, clearChangeLog };
