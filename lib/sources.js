...
// Hierarchy fields, in BIPO's own field/display order (broad -> narrow at the front, but see
// buildOrgData.js's EMPLOYEE_PATH_LEVELS for the actual depth order used to match against Lark
// Structures — Team/Sub Team/Section do NOT sit in depth order despite their names/positions here).
// "Business Unit" was removed from BIPO entirely (confirmed via the Base API's own field-not-found
// response) and replaced by "Sub Department", inserted after Department rather than in Business
// Unit's old spot after Division. Lark Structures gained a 7th level (L7_dept) at the same time, to
// carry a new "Group" tier inserted above a subset of what used to be flat, root-level divisions.
...
// These five are native Lark user-link fields on Structures (PIC directly; the HRBP*/Assistant
// ones via a lookup into "HOD & HRBP List", itself backed by user-link fields) — each cell carries
// a real open_id, not just a display name. The Snapshot Structures table mirrors that open_id into
// a same-named "<Field>_id" text column alongside the existing plain-text name column, so identity
// can be resolved precisely (matched against Lark User's own open_user_id) instead of by fuzzy name
// matching, which is ambiguous whenever two employees share a name and wrong whenever a short PIC
// name happens to be a substring of an unrelated person's name.
const ROLE_USER_FIELDS = ['PIC', 'HRBP1', 'HRBP2', 'HRBP Lead', 'Department Assistant'];
...
const SOURCES = {
  ...
  larkUsers: {
    ...
    // open_user_id is the same real Lark account id carried by the Structures table's PIC/HRBP*
    // user-link fields — the join key that lets a role value resolve to a specific EID instead of
    // a name string.
    // Company/Employee Type/Nationality/Email/DeptFullPath identify and place consultants and
    // shared/function accounts — people who exist here but never appear in the Employee list
    // (BIPO) roster at all, so they'd otherwise be invisible on the chart.
    // Employee/Consultant/Shared Account are formula fields the user maintains directly on this
    // table — the authoritative classification source (replacing the heuristic above): Employee
    // non-empty means this row already has a real Employee list (BIPO) record; otherwise
    // Consultant/Shared Account being marked (non-empty) says which of those two this is, and
    // status (Inactive vs not) splits everyone left over into pending-onboard vs undefined.
    fields: ['EID', 'Name', 'status', 'open_user_id', 'Company', 'Employee Type', 'Nationality', 'Email', 'DeptFullPath', 'Employee', 'Consultant', 'Shared Account'],
  },
  permissions: {
    key: 'permissions',
    tableId: 'tblX2q2qFshb3Hu0', // "Tool Permissions" — who can view/edit/admin this tool
    // Edit Scope: JSON array of department node ids this person is responsible for (picked from
    // the live org tree in the admin panel), stored as plain text same as changelog's Params.
    // Empty/absent means unrestricted — only used to filter that person's own "下载 CSV" exports,
    // never to gate what they can actually edit (see buildScopeClosure in app.js).
    fields: ['OpenID', 'Name', 'Email', 'Role', 'Edit Scope'],
  },
  settings: {
    key: 'settings',
...
module.exports = { BASE_TOKEN, SOURCES, EMPLOYEE_LEVELS, STRUCTURE_LEVELS, ROLE_USER_FIELDS };
