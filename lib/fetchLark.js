...
// fieldNamesOverride lets a caller ask for a subset of the source's own configured fields — used
// by listPermissionRecords to fall back when a newly-added field (e.g. "Edit Scope") doesn't
// exist in Base yet, without that failure taking down permission resolution (and with it, login)
// for everyone until an admin gets around to adding the field.
async function fetchSourceRecords(sourceKey, env = process.env, fieldNamesOverride) {
  const source = SOURCES[sourceKey];
  if (!source) throw new Error(`Unknown source: ${sourceKey}`);
  const { appId, appSecret } = requireCredentials(env);
  return searchAllRecords({ appId, appSecret, tableId: source.tableId, fieldNames: fieldNamesOverride || source.fields });
}
