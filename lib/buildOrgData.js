...
function buildOrgData(input) {
  ...
  function classifyLarkUser(f) {
    ...
    // Inactive means set up in Lark ahead of an actual start date, not onboarded yet; Active with
    // an EID but still missing from BIPO is very likely just a data-entry lag on BIPO's side.
    ...
    if (looksLikeRealHire && status === 'Active' && extractText(f.EID)) return 'missing';
    return null;
  }
