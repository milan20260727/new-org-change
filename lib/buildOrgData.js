...
function buildOrgData(input) {
  ...
  function classifyLarkUser(f) {
    ...
    if (company !== 'DigiPlus' && company !== 'Board of Directors') return null;
    ...
    // Profiled exactly like a real hire (Local/Expat, Regular/Probationary) but has no Employee
    // list (BIPO) record at all (the employeeEidSet check below is what actually enforces "not
    // already a tracked employee" — this branch alone doesn't verify that). Split by status:
    // Inactive means set up in Lark ahead of an actual start date, not onboarded yet; anything
    // else (Active, blank, or otherwise) with an EID but still missing from BIPO is very likely
    // just a data-entry lag on BIPO's side — not narrowed to exactly "Active" so an unusual status
    // value doesn't silently fall through neither bucket.
    const looksLikeRealHire = (nationality === 'Local' || nationality === 'Expat') && (empType === 'Regular' || empType === 'Probationary');
    const status = extractText(f.status);
    if (looksLikeRealHire && status === 'Inactive') return 'pending';
    if (looksLikeRealHire && status !== 'Inactive' && extractText(f.EID)) return 'missing';
    return null;
  }
  // A Lark User row that classifies as consultant/shared/pending but whose EID ALSO has a real,
  // active Employee list (BIPO) record is already properly represented as a real employee —
  // Employee list wins, so they're never double-shown here as well (a "Consultant"-typed Lark
  // account that's actually a tracked employee is just an employee).
