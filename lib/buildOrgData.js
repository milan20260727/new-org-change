...
function buildOrgData(input) {
  ...
  function classifyLarkUser(f) {
    ...
    // Set up in Lark ahead of their actual start date — Inactive (no access yet) but otherwise
    // profiled exactly like a real hire (Local/Expat, Regular/Probationary), so they're worth
    // showing on the chart even though the Employee list (BIPO) has nothing on them yet either.
    const status = extractText(f.status);
    if (status === 'Inactive' && (nationality === 'Local' || nationality === 'Expat') && (empType === 'Regular' || empType === 'Probationary')) return 'pending';
    return null;
  }
